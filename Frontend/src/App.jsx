...
const firebaseConfig = {
  ...
  storageBucket: "kasir-pintar-93e03.appspot.com", // Saya perbaiki sedikit dari .firebasestorage.app ke format standar React
...
function App() {
  ...
  const checkout = async () => {
    ...
    const dataTrans = {
      ...
      total: total,
      waktu: new Date() // Menyimpan tanggal transaksi
    };
    try {
  ...
  // =========================================================================
  // UI START (MENGGUNAKAN DESAIN BLACKBOX 100%)
  // =========================================================================
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", background: '#f8fafc' }}>
            ...
            <div style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
              {filteredTransaksi.map(t => (
                <div key={t.id} style={{ padding: '24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ fontWeight: 'bold', color: '#10b981', fontSize: '18px' }}>Rp {t.total.toLocaleString()}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {showCart && (
        <div className="no-print" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            <div style={{ padding: '32px 32px 24px', borderBottom: '1px solid #e2e8f0', position: 'sticky', top: 0, background: 'white', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div style={{ padding: '24px 32px' }}>
              {cart.map(item => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 0', borderBottom: '1px solid #f1f5f9' }}>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', fontWeight: '600', color: '#1e293b' }}>{item.nama}</h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#f1f5f9', padding: '8px 16px', borderRadius: '10px' }}>
                        <button onClick={() => updateQuantity(item.id, item.qty - 1)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#64748b' }}>-</button>
                        <span style={{ fontWeight: '600', color: '#1e293b', minWidth: '24px', textAlign: 'center' }}>{item.qty}</span>
                        <button onClick={() => addToCart(item)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#10b981' }}>+</button>
                      </div>
                      <span style={{ fontSize: '16px', fontWeight: '700', color: '#10b981' }}>Rp {(item.harga * item.qty).toLocaleString()}</span>
                    </div>
                  </div>
                  <button onClick={() => updateQuantity(item.id, 0)} style={{ background: '#fee2e2', border: 'none', width: '40px', height: '40px', borderRadius: '10px', color: '#dc2626', cursor: 'pointer', fontSize: '18px' }}>×</button>
                </div>
              ))}
            </div>

            <div style={{ padding: '24px 32px 32px', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', fontSize: '20px', fontWeight: '700' }}>
                <span>Total:</span>
                <span style={{ color: '#10b981' }}>Rp {cart.reduce((sum, item) => sum + (item.harga * item.qty), 0).toLocaleString()}</span>
              </div>
              <button onClick={checkout} disabled={cart.length === 0} style={{ width: '100%', padding: '18px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '600', cursor: cart.length === 0 ? 'not-allowed' : 'pointer', boxShadow: '0 10px 25px -5px rgba(16, 185, 129, 0.4)' }}>
                Bayar & Cetak Struk
              </button>
            </div>
          </div>
      ...
      {strukData && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div id="strukArea" style={{ background: '#fff', width: '300px', padding: '20px', textAlign: 'center', color: '#000', fontFamily: 'monospace' }}>
            <h3 style={{ margin: '0' }}>{namaToko || 'STRUK BELANJA'}</h3>
            <p style={{ fontSize: '10px', margin: '5px 0' }}>{alamat}<br/>Telp: {noTelp}</p>
            <div style={{ borderTop: '1px dashed #000', margin: '10px 0' }}></div>
            <p style={{ fontSize: '10px', textAlign: 'left' }}>Tgl: {strukData.waktu.toLocaleString()}</p>
            <div style={{ borderTop: '1px dashed #000', margin: '10px 0' }}></div>
            ...
            <div className="no-print" style={{ marginTop: '20px' }}>
              <button onClick={() => window.print()} style={{ background: '#10b981', color: '#fff', padding: '10px 15px', border: 'none', marginRight: '10px', borderRadius: '8px', cursor: 'pointer' }}>Print</button>
              <button onClick={() => setStrukData(null)} style={{ background: '#e2e8f0', padding: '10px 15px', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Tutup</button>
            </div>
          </div>
        </div>
      )}
